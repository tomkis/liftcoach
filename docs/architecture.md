...
React Native + Expo, TypeScript, SQLite + Drizzle ORM, tRPC (local, no network), TanStack React Query, Zod, React Navigation. watchOS companion app in SwiftUI.
...
The `ios/` directory is ejected (version-controlled, not regenerated by Expo prebuild). This was necessary to support the native watchOS target.

- **Main app**: `ios/DEVLiftCoach/` — React Native + Expo
- **Watch app**: `ios/LiftCoachWatch/` — standalone SwiftUI app embedded in the main app
- Dev builds use `com.liftcoach.app.dev` bundle ID to coexist with prod on the same device

## Development

```bash
pnpm start          # run Metro bundler
pnpm typecheck      # always run after changes
pnpm lint           # always run after changes
pnpm knip           # check for unused exports/deps/files
```

Build and run from Xcode using the `DEVLiftCoach` scheme. The `LiftCoachWatch` scheme can be used to run the watch app independently on a watch simulator.
